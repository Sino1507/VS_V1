if not syn or not syn.request then appendfile("VS_V1/log.txt", "Unsupported Exploit... (ErrorCode:1)!\n")  selfObj:Kick("Unsupported Exploit... (ErrorCode:1)!\n") end


local selfObj, charObj = game.Players.LocalPlayer, game.Players.LocalPlayer.Character
...
function decode(x)
    return json:JSONDecode(x)
end

function encode(x)
    return json:JSONEncode(x)
end


function log(msg)
    return appendfile("VS_V1/log.txt" , msg.."\n")
end


local publicrepo = "https://raw.githubusercontent.com/Sino1507/VS_V1/main/"


function getRepoModule(ModuleRepo, module)
    local res = game:HttpGet(ModuleRepo..module, true)
...
local IfSettings_Prefix = isfile("RA_Settings_Prefix.lua")


if IfSettings_Prefix == false then
    writefile("RA_Settings_Prefix.lua",'.')
end
...
local LastTeamCode = nil

states.AUTO_SAVE = true


local version = "v1.41"
local prefix = tostring(readfile("RA_Settings_Prefix.lua"))
...
ScorpionAdmin.Name = "Raven Admin"
...
Title_3.Text = "Raven Admin-Loader "..version
...
Logo_3.Image = "http://www.roblox.com/asset/?id=9176462045"
...
Title.Text = "Raven Admin-Commands"
...
Logo.Image = "http://www.roblox.com/asset/?id=9176462045"
...
Title_2.Text = "Raven Admin-Credit"
...
TextLabel_2.Text = "Made By Sino#4162 aka Silas."
...
Logo_2.Image = "http://www.roblox.com/asset/?id=9176462045"
...
Logo_4.Image = "http://www.roblox.com/asset/?id=9176462045"
...
Title_5.Text = "Raven Admin-Output"
...
Logo_5.Image = "http://www.roblox.com/asset/?id=9176462045"
...
allcmds = {
	...
	"tp [plr] [plr] - Teleports player to another player.",
	"bring [plr] - Teleports player to you.",
	"mbring [multiple plrs] - Teleports multiple players to you.",
	"refresh [plr] - Refreshes a player.",
	"give [plr] [item] - Gives a player an item.",
	"getteam [plr] - Gets a player's team.",
	"shield - Gives shield.",
	"armor - Gives armor.",
	"kit - Gives kit.",
	"rj - Rejoins the game.",
    ...
    "god - god mode but you look damn ugly.",
    "ungod - Resets god mode.",
	"",
	"--- Server ---",
	...
	"loopkill / lk [plr, _i, _g, _c, _n, _s] - Loopkill player.",
	"unloopkill / unlk [plr] - UnLoopkill player.",
	"clearloopkill / clk - Clears all Loopkill.",
	...
	"sport [plr] - Puts them to the gym.",
	"lunch [plr] - Puts them in the caferitia.",
	"spin [speed] - Spins you.",
	"unspin - Unspins you.",
	"",
	"--- Toggles ---", 
	"antiswat - Anti-Swat.",
	"antiweapon [plr] [on/off] - Kills the plr if he has a weapon/tool.",
	"anticheat - Anti-Cheat.",
	"autosave - If on all values like prefix etc. will be saved.",
	...
	"",
	"--- Other ---",
	"infadmin - loads Infinite Yield.",
	"save - saves all current values like prefix etc.",
	...
	"savecframe - saves current frames.",
	"getusers - gets Raven Admin users.",
	"",
...
function changeprefix(prefix_text)
	...
	if states.AUTO_SAVE == true then
		writefile("RA_Settings_Prefix.lua",prefix)
	end
	Input.PlaceholderText = "Press "..prefix.." to focus command bar."
end
...
function cgun(item)
	if item:lower() == "m9" or item:lower() == "pistole" then
		workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.giver["M9"].ITEMPICKUP)
	elseif item:lower() == "ak" or item:lower() == "ak47" or item:lower() == "ak-47" then
		workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.giver["AK-47"].ITEMPICKUP)
	elseif item:lower() == "m4" or item:lower() == "m4a1" or item:lower() == "m4a1-s" then
		workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.giver["M4A1"].ITEMPICKUP)
	elseif item:lower() == "remington" or item:lower() == "rem" or item:lower() == "r" or item:lower() == "shotgun" then
		workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.giver["Remington 870"].ITEMPICKUP)
	elseif item:lower() == "shield" or item:lower() == "s" then
		workspace.Remote.TeamEvent:FireServer("Bright blue")
		workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.giver["Riot Shield"].ITEMPICKUP)
	end
end

function give(target,item)
	...
	if target ~= nil then
		local NormPos = speaker.Character.HumanoidRootPart.CFrame
		savecf = speaker.Character.HumanoidRootPart.CFrame
		if not speaker.TeamColor == ("Bright blue") then
			workspace.Remote.loadchar:InvokeServer()
		end
		cgun(item)
		wait()
		...
		local char = speaker.Character
		local tchar = target.Character
		local savedtpos = tchar.HumanoidRootPart.CFrame
		local hum = speaker.Character:FindFirstChildOfClass("Humanoid")
		local hrp = speaker.Character.HumanoidRootPart
		local hrp2 = target.Character.HumanoidRootPart
		hum.Name = "1"
...
function chatnotify(text)
	game.StarterGui:SetCore("ChatMakeSystemMessage", {Text = text; Color = Color3.new(1,0,0); Font = Enum.Font.SourceSans; FontSize = Enum.FontSize.Size42})
end
...
chatnotify("While being in a ColoredTeam dont equippe a gun otherwise ur going to be a crim.")
chatnotify("Im going to fix that as fast as i can.")
...
function LocalPlayerChatted(msg)
	...
	if command("brag") then
		chat("Raven Admin!! Made by Sino_1507 aka Silas and the ui was made by Vampin'cat.")
	...
	if command("getteam") then
		--check if there is a arg2 if not return
		if not arg2 then
			notify("No team player.")
			return
		end

		--get the TeamColor and put it in a variable called team
		local team = GetPlayerFromString(arg2).TeamColor
		--check if the team isnt nil if it is return
		if not team then
			notify("No team player.")
			return
		end
		--notify the TeamColor and the player by saying "player is in team"
		notify(GetPlayerFromString(arg2).Name.." is in team "..team)
	end
	if command("give") or command("gun") or command("nodamagegun") or command("tool") then
		if not arg2 then
			notify("No player.")
			return
		end
	...
	if command("key") then
		workspace.Remote.TeamEvent:FireServer("Bright blue")
		game.Player.LocalPlayer:LoadCharacter()
	end
	...
	if command("crash") then
		...
		local PackageSize = 10000 -- How many bullets is sent through in one remote call. Thanks Globals
		local SendPackageAmountOfTimes =  10 -- How many times the remote should be called.
	...
	if command("autosave") then
		if not arg2 then notify("No state was called(off/on).") return end
		states.AUTO_SAVE = getBoolFromString(arg2)

		notify("AutoSave was set "..arg2)
	end
	if command("save") then
		writefile("RA_Settings_Prefix.lua",prefix)

		notify("Saved all Values.")
	end
    if command("god") or command("godmode") then
		states.God_Mode = true
		notify("Turn god mode on.")
		while wait() do
	...
	if command("armory") or command("arm") then
		game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(782.77533, 99.9900055, 23133613)
	end
	...
	if command("anticrash") then
		if getBoolFromString(arg2) then
            ...
            notify("Also turned new anticrash",arg2..".")
		else
			EnableShootEvent()
			notify("Turned anticrash",arg2..".")
		end
	...
	if command('getusers') then
		...
		for _,v in pairs(game.Players:GetPlayers()) do
			if IsInGroup(v.UserId,8325547) or IsFriendsWith(v.UserId,game.Players.LocalPlayer.UserId) then
				chatnotify(v.Name)
			end
		end
	end
	...
	if command("loopkill") or command("lk") then
		local function addPlayer(name)
			local player = GetPlayerFromString(name)
			if player and not Loopkill[player.UserId] and player ~= game.Players.LocalPlayer then 
	...
	if command("infadmin") then
		loadstring(game:HttpGet(('https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source'),true))()
		notify("InfAdmin loaded!")
	end
	if command("sport") then
		if not arg3 then 
			teleport(GetPlayerFromString(arg2),CFrame.new(770, 101, 2537))
		end

		if getBoolFromString(arg3) then
			teleport(GetPlayerFromString(arg2),CFrame.new(770, 101, 2537))
			chat("Go do some sport it's healty.")
		else
			teleport(GetPlayerFromString(arg2),CFrame.new(770, 101, 2537))
		end
	end
	if command("endserver") then
		notify("Ending the server...")
	...
	if command("tp") then
		--check if two players a defined
		if not arg2 then
			notify("Please define a player.")
			return
		end
		if not arg3 then
			notify("Please define a player to tp to.")
			return
		end
		--getFirstPlayer using local First
		local First = GetPlayerFromString(arg2)
		--getSecondPlayer using local Second
		local Second = GetPlayerFromString(arg3)

		--teleport First to Second
		teleport(First,Second.Character.HumanoidRootPart.CFrame)

		--notify a successful teleport
		notify("Teleported "..First.Name.." to "..Second.Name)
	end
	if command("refresh") then
		...
		--get the definded player
		local Player = GetPlayerFromString(arg2)
		--save the player's current cframe
		local CFrame = Player.Character.HumanoidRootPart.CFrame
		--kill the player and wait for the player to respawn
		kill(Player)
		wait(5)
		--Teleport the player to the saved cframe
		teleport(Player,CFrame)
		--notify a successful refresh
		notify("Refreshed "..Player.Name)
	end
	if command("mbring") then

		--get the length of the arguments
		local Length = #split_message

		--loop through the arguments
		for i = 2, Length do
			--get the player from the argument
			local Player = GetPlayerFromString(split_message[i])
			--bring the player to the local player
			bring(Player)
		end

		--notify a successful teleport
		notify("Brought "..#length.." players.")
	end
	if command("savecframe") then
		--get the local player
		local Player = game.Players.LocalPlayer
		--get the local player's cframe
		local CFrame = Player.Character.HumanoidRootPart.CFrame
		
		--copy the cframe to the clipboard
		setclipboard(CFrame)

		--notify a successful save
		notify("Copied cframe to clipboard.")
	end
	if command("antiswat") then
		--set states.ANTI_SWAT to getBoolFromString(arg2)
		states.ANTI_SWAT = getBoolFromString(arg2)
		--notify a successful antiswat
		notify("Set ANTI_SWAT to "..arg2)
	end
	if command("spin") then
		--make a variable called Spinspeed
		local Spinspeed = tonumber(arg2)

		--get the local player character
		local Character = game.Players.LocalPlayer.Character.HumanoidRootPart

		--[[
			creat a BodyAngularVelocity called local Spin
			set the parent to the local player character
			set the angular velocity y to the Spinspeed
		]]
		local Spin = Instance.new("BodyAngularVelocity")
		Spin.Name = "Spin"
		Spin.Parent = Character
		Spin.MaxTorque = Vector3.new(0,math.huge,0)
		Spin.AngularVelocity = Vector3.new(0,Spinspeed,0)
		--notify a successful spin
		notify("Spinning "..Spinspeed)
	end
	if command("unspin") then
		--get the local player character
		local Character = game.Players.LocalPlayer.Character
		--get the spin from the character
		local Spin = Character.HumanoidRootPart:FindFirstChild("Spin")
		--if the spin exists
		if Spin then
			--destroy the spin
			Spin:Destroy()
		...
		workspace.CurrentCamera.CFrame = lastCam
		game.StarterGui:SetCoreGuiEnabled(Enum.CoreGuiType.Backpack, true)
		--notify a successful unspin
		notify("Unspinning")
	end
	if command("anticheat") then
		--set states.ANTI_CHEAT to getBoolFromString(arg2)
		states.ANTI_CHEAT = getBoolFromString(arg2)
		--notify a successful anticheat
		notify("Set ANTI_CHEAT to "..arg2)
	end
	if command("antigun") or command("antiweapon") or command("antitool") then
		if not arg2 then
			notify("Please define a player.")
			return
		end
		if not arg3 then
			notify("Please define on/off.")
			return
		end
		--get the player from the argument
		local Player = GetPlayerFromString(arg2)
		...
		if Player == game.Players.LocalPlayer then
			notify("You can't disable your own gun.")
			return
		end
		--insert the player into the antigun table
		--create a variable that defines a boolean using getBoolFromString(arg3)
		local bool = getBoolFromString(arg3)
		--if the player is in the antigun table and the boolean is false then remove the player from the antigun table
		if bool == false then
			table.remove(AntiGun,table.find(AntiGun,Player))
		else
			table.insert(AntiGun,Player)
		end

		--notify a successful AntiGun
		notify("Set AntiGun "..Player.Name.." to "..arg3)
	end
end
...
spawn(function()
	while wait() do
		for i,v in pairs(AntiGun) do
			pcall(function()
				--check if the user has a tool in their backpack and if they do, kill them
				if v and v.Character and v.Character.Head and v.Character.Humanoid.Health ~= 0 then
					for i,v2 in pairs(v.Backpack:GetChildren()) do
						if v2:IsA("Tool") then
							kill(v)
						end
					end
...
table.insert(connections, game:GetService("RunService").RenderStepped:Connect(function()
	...
	if states.ANTI_SWAT then
		for _,v in pairs(game.Players:GetPlayers()) do
			pcall(function()
				if v == game.Players.LocalPlayer then
					return
				end
				if game:GetService("Workspace")[v.Name].vest or game:GetService("Workspace")[v.Name]["Riot helmet"] or game:GetService("Workspace")[v.Name]["Ski mask"] or game:GetService("Workspace")[v.Name].Torso.ShieldFolder then
					kill(v)
				end
			end)
		end
	end
	if states.ANTI_CHEAT then
		for _,v in pairs(game.Players:GetPlayers()) do
			pcall(function()
				--[[
					check if v team is not equal to inmate or guard or criminal
					if it is not then kill them
				]]
				if v.TeamColor.Name ~= ('Really red' or 'Bright orange' or 'Bright blue' or 'Medium stone grey') then
					if v == game.Players.LocalPlayer then
						return
					else
						Loopkill[player.UserId] = { player = player } 
	...
	if states.ANTI_GUN then
		--check if a user has a gun in their backpack
	end
